    ...
    with TaskGroup(group_id="load_stage") as load_stage:
        with TaskGroup(group_id="load_dimensions_step") as load_dimensions_step:
            ...
            load_incident_types = custom_livy_operator(
                ...
                args=[
                    "s3a://dwp/staging/incident-type-dimension.parquet",
                    "default.incident_type_dimension",
                ],
            )
            load_dimensions_step = MetaTask("load_dimensions_step", dag).wrap(
                [load_date_dimension, load_incident_types]
            )

        load_stage = MetaTask("load_stage", dag).wrap_steps([load_dimensions_step])

    transform_stage.set_upstream(extract_stage)
    load_stage.set_upstream(transform_stage)
